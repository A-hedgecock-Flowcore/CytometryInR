...
For this one, we start off with one of the cosine matrices we generated previously. Lets also go ahead and store the fluorophore names as a a vector so that we can append them back in later on. 
...
At this point comes the math portion of the code. 
...
At which point we can return the Fluorophore vector of names to their original rownames position. We can also round the values to make interpretation a bit easier in the larger plot. 
...
For unmixing-dependent spread matrix, it is normally presented as a triangle, so we can work use base R notation to set the upper half of the triangle to NA values. 
...
From here, we can convert the matrix back to a data.frame object, copy the rownames to the columns, and make the rownames a proper column as well through the use of the `tibble` packages `rownames_to_column()` function. 
